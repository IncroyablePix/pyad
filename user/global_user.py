...
class GlobalUser:
	...
	def register(self, gid: int = None, ldappasswd: str = None):
		...
		result = subprocess.run(f"""/usr/bin/ldapadd {options} -D 'cn=Directory Manager,dc=localdomain' -f {GlobalUser.insert_file} -x""", shell = True)
		return result

	
	def add_to_group(self, global_group: GlobalGroup, ldappasswd: str = None):
		with open(GlobalUser.add_member_file, """w""") as temp_file:
			temp_file.write(f"""dn: cn={global_group.group_name},ou={global_group.ou_name},dc=localdomain\n""")
		...
		result = subprocess.run(f"""/usr/bin/ldapadd {options} -D 'cn=Directory Manager,dc=localdomain' -f {GlobalUser.add_member_file} -x""", shell = True)
		return result
